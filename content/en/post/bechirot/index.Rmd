...
bechirot <- bechirotraw %>% mutate(envelopes = if_else(place %in% c(0, 875, 9999, 99999), 1, 0))
population_per_election <- bechirot %>% filter(envelopes == 0) %>% group_by(elections) %>% summarize(pop = sum(votes))
votes_per_election <- bechirot %>% filter(party != "didntvote") %>% group_by(elections, envelopes) %>% summarize(tv = sum(votes)) %>% inner_join(population_per_election) %>% mutate(turnout = tv/pop)
votes_per_party <- bechirot %>% filter(party != "didntvote") %>% group_by(elections, party) %>% summarize(pv = sum(votes)) %>% inner_join(population_per_election) %>% mutate(voteshare = pv/pop)
```
